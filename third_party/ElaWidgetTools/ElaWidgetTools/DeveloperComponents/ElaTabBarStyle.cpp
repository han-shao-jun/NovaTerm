    ...
    case QStyle::CE_TabBarTabShape:
    {
        ...
        if (const QStyleOptionTab* topt = qstyleoption_cast<const QStyleOptionTab*>(option))
        {
            ...
            if (topt->state.testFlag(QStyle::State_Selected))
            {
                ...
                {
                    int indicatorX = tabRect.left() + horizontalPadding;
                    int indicatorWidth = tabRect.width() - 2 * horizontalPadding;
                    int indicatorY = (_pIndicatorPosition == ElaTabBarType::Top) ? tabRect.y() + 3 : tabRect.y() + tabRect.height() - indicatorThickness - 3;
                    const ElaTabBar* tabBar = qobject_cast<const ElaTabBar*>(widget);
                    if (tabBar)
                    {
                        if (!tabBar->getIsIndicatorAnimationFinished())
                        {
                            indicatorX = tabBar->getIndicatorX();
                            int animWidth = tabBar->getIndicatorAnimationWidth();
                            if (animWidth > 0)
                            {
                                indicatorWidth = animWidth;
                            }
                        }
                    }
                    painter->drawRoundedRect(QRectF(indicatorX, indicatorY, indicatorWidth, indicatorThickness), indicatorRadius, indicatorRadius);
                    break;
                }
                }
            }
