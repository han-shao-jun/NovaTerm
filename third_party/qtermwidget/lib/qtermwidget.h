...
public:
    ...
    /**
     * Send data directly to the terminal emulation,
     * bypassing the PTY layer. This is useful on platforms
     * where PTY is not available (e.g. Windows).
     */
    void receiveData(const char* buffer, int len) override;
